...
exports.create = function(user, type, entity, callback) {
    var hashCode = hash(entity.longUrl);
    var length = entity.longUrl.length;
        ...
        if (error && callback) {
            ...
            if (result.length >= 1 ) {
                for(var i in result) {
                    ...
                    if (element.longUrl == entity.longUrl) {
                        callback(error, element);
                        break;
                    }
                }
            } else {
                var shortUrlEntity = {longUrl: entity.longUrl, hashCode: hashCode, length: length};
